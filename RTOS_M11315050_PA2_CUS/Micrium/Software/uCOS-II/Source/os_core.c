    ...
    if (OSRunning == OS_TRUE) {
        ...
        if (OSIntNesting == 0u) {                          /* Reschedule only if all ISRs complete ... */
            if (OSLockNesting == 0u) {                     /* ... and not locked.                      */
                ...
                for (INT16U i = 0;i < Aperiodic_NUMBER;i++) {
                    if (Aperiodic_TaskParameter[i].TaskArriveTime == OSTimeGet()) {
                        ...
                        if (Max_DeadLine + Aperiodic_TaskParameter[i].TaskExecutionTime * 100 / Sever_U <= Aperiodic_TaskParameter[i].TaskPeriodic) {
                            ...
                            if (Rdyhead == NULL) {
								...
								Rdyhead = newPtr;
                                Rdyhead->data = &Aperiodic_TaskParameter[i];
								Rdyhead->link = NULL;
                            }
                            else {
                                NodePtr t_node = Rdyhead;
								while (t_node->link != NULL)
								{
									t_node = t_node->link;
								}
                                NodePtr newPtr = (NodePtr)malloc(sizeof(Node));
								newPtr->data = &Aperiodic_TaskParameter[i];
								newPtr->link = NULL;
								t_node->link = newPtr;
                            }
                            if (OSTCBPrioTbl[Sever_Priority]->OSTCBDly == 0) {
                                printf("%2d\tAperiodic job(%d) arrives and sets CUS sever's deadline as %d.\n", OSTime, Aperiodic_TaskParameter[i].TaskID,OSTime + Aperiodic_TaskParameter[i].TaskExecutionTime * 100 / Sever_U);
                                fprintf(Output_fp,"%2d\tAperiodic job(%d) arrives and sets CUS sever's deadline as %d.\n", OSTime, Aperiodic_TaskParameter[i].TaskID, OSTime + Aperiodic_TaskParameter[i].TaskExecutionTime * 100 / Sever_U);
                            ...
                            printf("%2d\tAperiodic job(%d) arrives. But can't Scheduled. (miss absoulute deadline)\n", OSTime, Aperiodic_TaskParameter[i].TaskID);
                            fprintf(Output_fp,"%2d\tAperiodic job(%d) arrives. But can't Scheduled. (miss absoulute deadline)\n", OSTime, Aperiodic_TaskParameter[i].TaskID);

                        }

                    }
                }
                if (OSTCBPrioTbl[Sever_Priority]->OSTCBDly == 0 && Rdyhead != NULL) {
                    if (Rdyhead->data->TaskArriveTime != OSTime) {
                        printf("%2d\tAperiodic job(%d) sets CUS sever's deadline as %d.\n", OSTime, Rdyhead->data->TaskID, OSTime + Rdyhead->data->TaskExecutionTime * 100 / Sever_U);
...
void  OSTimeTick (void)
{
    ...
    if (OSRunning == OS_TRUE) {
        ...
        while (ptcb->OSTCBPrio != OS_TASK_IDLE_PRIO) {     /* Go through all TCBs in TCB list              */
            ...
            if (ptcb->OSTCBDly == 0u && ptcb->OSTCBPrio != Sever_Priority) {
                OS_ENTER_CRITICAL();
            ...
            if(DEBUG)
                printf("TASK %2d: TCBDly %d  Exec %d Work_Load %d\n", ptcb->OSTCBId, ptcb->OSTCBDly, ptcb->TaskExecutionTime, ptcb->TASKWorkLoad);
...
static  void  OS_SchedNew (void)
{
    ...
    ptcb = OSTCBList;
    INT16U MIN_ID=0XFFFF,MAX_Arrived= 0XFFFF;
	BOOLEAN chosed = 0;
    while (ptcb != (OS_TCB*)0) {
        ...
        if (OSTime == 0) {
            if (inRdyGrp && inRdyTbl&& ptcb->TASKWorkLoad!=0) {
				chosed = 1;
                if (DEBUG)
            ...
            if (inRdyGrp && inRdyTbl && ptcb->TASKWorkLoad != 0) {
                ...
                printf("Task @%p (prio=%2d) is READY %d %d %d\n", ptcb, ptcb->OSTCBId, ptcb->TaskPeriodic, ptcb->OSTCBDly, ptcb->TaskPeriodic - ptcb->OSTCBDly);
                chosed = 1;
                if (ptcb->OSTCBDly < MAX_Arrived&& ptcb->OSTCBDly != 0) {
                    MAX_Arrived =  ptcb->OSTCBDly;
                    MIN_ID = ptcb->OSTCBId;
                    OSPrioHighRdy = prio;
                }
                ...
                else if (ptcb->OSTCBDly == 0 && ptcb->TaskPeriodic <= MAX_Arrived && prio!=63 && ptcb->OSTCBId < MIN_ID) {
                    MAX_Arrived = ptcb->TaskPeriodic;
                    MIN_ID = ptcb->OSTCBId;
                    OSPrioHighRdy = prio;
                }
                else if (prio==63&& MAX_Arrived==0XFFFF) {
                    OSPrioHighRdy = prio;
                }
            }
            else {
                   ...
                   INT16U   opt)
{
    ...
    if (ptcb != (OS_TCB *)0) {
        ...
        static INT16U taskcreated = 0;
        taskcreated++;
        printf("Task[%2d] created, TCB Address %p\n", ptcb->OSTCBPrio, ptcb);
        ...
        if (taskcreated == TASK_NUMBER + 1 && TASK_NUMBER != 0) {
            OS_TCB* p = OSTCBList;
            printf("\n==================TCB linked list===================\n");
