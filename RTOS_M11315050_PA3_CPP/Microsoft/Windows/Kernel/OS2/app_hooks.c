...
void InputFile() {
    ...
    while (!feof(fp)) {
        ...
        while (ptr != NULL) {
            ...
            if (i == 0) {
                ...
                TaskParameter[j].lockR1 = TaskInfo[i];
            }
            else if (i == 5) {
                TaskParameter[j].unlockR1 = TaskInfo[i];
            }
            else if (i == 6) {
                TaskParameter[j].lockR2 = TaskInfo[i];
            }
            else if (i == 7) {
                TaskParameter[j].unlockR2 = TaskInfo[i];
            }
            i++;
        }
    ...
    for (int m = 0; m < TASK_NUMBER; m++) {
        TaskParameter[m].TaskPriority = 3+m*3;  
    }
	...
	for (int i = 0; i < TASK_NUMBER; i++) {
        ...
        if (TaskParameter[i].unlockR1 != 0&& TaskParameter[i].TaskPriority<R1_PRIO) {
			R1_PRIO = TaskParameter[i].TaskPriority-1;
        }
        if (TaskParameter[i].unlockR2 != 0 && TaskParameter[i].TaskPriority < R2_PRIO) {
			R2_PRIO = TaskParameter[i].TaskPriority-2;
        }
    }
	printf("R1_PRIO: %d, R2_PRIO: %d\n", R1_PRIO, R2_PRIO);
    fclose(fp);
    /* read file */
}
/*
