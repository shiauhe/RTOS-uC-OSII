...
void  OSTimeTick (void)
{
    ...
    printf("\nTICK %d TASK %d R_USED %d\n", OSTime, OSTCBCur->OSTCBId, R_USED);
    if (OSRunning == OS_TRUE) {
#if OS_TICK_STEP_EN > 0u
...
static  void  OS_SchedNew (void)
{
    ...
    temp = (INT8U)((y << 3u) + OSUnMapTbl[OSRdyTbl[y]]);
    if ((temp != 63 && (temp < R_USED|| ((OS_TCB*)R1->OSEventPtr== OSTCBPrioTbl[temp] || (OS_TCB*)R2->OSEventPtr == OSTCBPrioTbl[temp])))||temp==63)
